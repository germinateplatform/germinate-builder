  ...
  --%>
	...
	$(document).ready(function () {
		...
		$('#builder-form').submit(function (event) {
			...
			var selectedInstances = $('select[name=<%= GerminateBuilderServlet.QUERY_PARAM_INSTANCES %>] option:selected').size();

			// Check if at least one instance is selected
			if (selectedInstances < 1) {
				$('#notifications').empty().append('<div class="alert alert-warning" role="alert">Please select at least one Germinate instance.</div>');
			}
			else {
				// get storage
				var storedData = getStorage('germinate-builder-');
