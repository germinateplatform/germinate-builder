...
<%
	...
	String svnInfo = VersionControlUtils.getInfo(PropertyReader.getProperty(PropertyReader.PATH_GERMINATE));
%>
...
<head>
	...
	<script src="js/build-germinate-js.jsp"></script>
...
<div class="container" style="position: relative;">
				...
				<h4 class="panel-title">
					<a role="button" data-toggle="collapse" data-parent="#svn-accordion" href="#collapseOne" aria-expanded="false"
					   aria-controls="collapseOne">
						Show Germinate version information
					</a>
				</h4>
			</div>
	...
	<form id="builder-form" action="germinate-builder">
		<div class="form-group">
			<label for="gatekeeper-instances">Select which Germinate instances to build</label>

			<select multiple class="form-control" id="gatekeeper-instances" name="<%= GerminateBuilderServlet.QUERY_PARAM_INSTANCES %>">
				<%
					try
					{
						User user = (User) request.getSession().getAttribute("user");

						for (Instance instance : InstanceManager.getAllForType(InstanceType.GERMINATE, user))
						{
		...
		<div class="form-group">
			<label for="translations">Check if you want to download the latest translations</label>

			<div class="checkbox">
				<label> <input type="checkbox" id="translations" name="<%= GerminateBuilderServlet.QUERY_PARAM_DOWNLOAD_TRANSLATIONS %>" value="true">Download
					translations?</label>
			</div>
		</div>
